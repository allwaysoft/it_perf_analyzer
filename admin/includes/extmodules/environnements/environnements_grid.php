...
Toc.environnements.environnementsGrid = function(config) {
  var that = this;
  config = config || {};
  ...
  config.ds = new Ext.data.Store({
    ...
    baseParams: {
      ...
      action: 'list_environnements'
    },
    reader: new Ext.data.JsonReader({
      root: Toc.CONF.JSON_READER_ROOT,
      totalProperty: Toc.CONF.JSON_READER_TOTAL_PROPERTY,
      id: 'environnements_id'
    }, [
      'environnements_id',
      'content_status',
      'content_order',
  ...
  config.cm = new Ext.grid.ColumnModel([
    config.sm,
    { id: 'content_name', header: '<?php echo $osC_Language->get('table_heading_environnements'); ?>', dataIndex: 'content_name', sortable: true},
  ...
  Toc.environnements.environnementsGrid.superclass.constructor.call(this, config);
};

Ext.extend(Toc.environnements.environnementsGrid, Ext.grid.GridPanel, {
  
  onAdd: function() {
    var dlg = this.owner.createenvironnementsDialog();
  ...
  onEdit: function(record) {
    ...
    dlg.show(record.get("environnements_id"),path);
  },
  
  onDelete: function(record) {
    var environnementsId = record.get('environnements_id');
        ...
        if (btn == 'yes') {
          Ext.Ajax.request({
            ...
            params: {
              ...
              environnements_id: environnementsId
            },
  ...
  onClick: function(e, target) {
    ...
    if (row !== false) {
      ...
      if (action != 'img-button') {
        var environnementsId = this.getStore().getAt(row).get('environnements_id');
        ...
        switch(action) {
          ...
          case 'status-on':
            ...
            this.onAction(module, environnementsId, flag);
  ...
  onAction: function(action, environnementsId, flag) {
    Ext.Ajax.request({
      url: Toc.CONF.CONN_URL,
      params: {
        module: 'environnements',
      ...
      callback: function(options, success, response) {
        ...
        if (result.success == true) {
          var store = this.getStore();
          store.getById(environnementsId).set('content_status', flag);
          store.commitChanges();
