<?php
  echo 'Ext.namespace("Toc.databases_dashboard");';
?>

Ext.override(TocDesktop.DatabasesDashboardWindow, {

    createWindow: function () {
        var desktop = this.app.getDesktop();
        var win = desktop.getWindow('databases_dash-win');

        if (!win) {
            //var grd = new Toc.DatabasesGrid({owner: this});
            //var availability_grd = new Toc.databasesAvailabilityDashboard({owner: this});
            var dashboard = new Toc.DatabaseDashboard({owner: this,header : false});
