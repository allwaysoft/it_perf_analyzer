...
Toc.articles.ArticlesGrid = function(config) {
  ...
  config.cm = new Ext.grid.ColumnModel([
    config.sm,
    { id: 'content_name', header: '<?php echo $osC_Language->get('table_heading_articles'); ?>', dataIndex: 'content_name', sortable: true},
...
Ext.extend(Toc.articles.ArticlesGrid, Ext.grid.GridPanel, {
  ...
  onEdit: function(record) {
    ...
    dlg.show(record.get("articles_id"),path);
  },
  ...
  onAction: function(action, articlesId, flag) {
    Ext.Ajax.request({
      ...
      callback: function(options, success, response) {
        ...
        if (result.success == true) {
          var store = this.getStore();
          store.getById(articlesId).set('content_status', flag);
          store.commitChanges();
