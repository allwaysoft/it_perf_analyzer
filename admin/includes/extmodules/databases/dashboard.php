...
Toc.databases.Dashboard = function(config) {

  config = config || {};
  
  config.id = 'databases-win';
  config.title = '<?php echo $osC_Language->get('heading_title'); ?>';
  config.layout = 'border';
  config.width = 1000;
  config.height = 500;
  config.iconCls = 'icon-dashboard-win';
  config.items = [this.getPortal()];
  ...
  Toc.databases.Dashboard.superclass.constructor.call(this, config);
};

Ext.extend(Toc.databases.Dashboard, Ext.Window, {

  show: function() {
    Toc.databases.Dashboard.superclass.show.call(this);
    this.loadPortlets();
    this.maximize();
  },
