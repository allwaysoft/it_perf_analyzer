...
Toc.cartes.clientsGrid = function(config) {
  var that = this;
  config = config || {};
  ...
  config.cm = new Ext.grid.ColumnModel([
    config.sm,
    { id: 'content_name', header: '<?php echo $osC_Language->get('table_heading_cartes'); ?>', dataIndex: 'content_name', sortable: true},
  ...
  Toc.cartes.clientsGrid.superclass.constructor.call(this, config);
};

Ext.extend(Toc.cartes.clientsGrid, Ext.grid.GridPanel, {
  
  onAdd: function() {
    var dlg = this.owner.createcartesDialog();
  ...
  onEdit: function(record) {
    ...
    dlg.show(record.get("cartes_id"),path);
  },
  
  onDelete: function(record) {
    var cartesId = record.get('cartes_id');
  ...
  onClick: function(e, target) {
    ...
    if (row !== false) {
      ...
      if (action != 'img-button') {
        var cartesId = this.getStore().getAt(row).get('cartes_id');
        ...
        switch(action) {
          ...
          case 'status-on':
            ...
            this.onAction(module, cartesId, flag);
  ...
  onAction: function(action, cartesId, flag) {
    Ext.Ajax.request({
      url: Toc.CONF.CONN_URL,
      params: {
        module: 'cartes',
      ...
      callback: function(options, success, response) {
        ...
        if (result.success == true) {
          var store = this.getStore();
          store.getById(cartesId).set('content_status', flag);
          store.commitChanges();
