...
Toc.cartes.cartesAmplitudeGrid = function(config) {
...
'sit'
  ...
  config.rowActions = new Ext.ux.grid.RowActions({
    actions:[
      {iconCls: 'icon-delete-record', qtip: 'Desactiver'}],
  ...
  config.cm = new Ext.grid.ColumnModel([
    ...
    { id: 'nctr',header: 'No Contrat', dataIndex: 'nctr',width:15,align: 'center'},
    { id: 'ncart',header: 'No Carte', dataIndex: 'ncart',width:30,align: 'center'},
    { id: 'cli',header: 'Code Client', dataIndex: 'cli',width:15,align: 'center'},
    { id: 'client',header: 'Client', dataIndex: 'nom',width:25},
    { id: 'ncp', header: 'No Compte', dataIndex: 'ncp',align: 'center',width:15},
    config.rowActions
  ]);
...
this.onSearch();
  ...
  Toc.cartes.cartesAmplitudeGrid.superclass.constructor.call(this, config);
};

Ext.extend(Toc.cartes.cartesAmplitudeGrid, Ext.grid.GridPanel, {

  onDelete: function(record) {
    var ncart = record.get('ncart');
    var age = record.get('age');
    var nctr = record.get('nctr');

    Ext.MessageBox.confirm(
      TocLanguage.msgWarningTitle,
      'Voulez-vous vraiment annuler la carte ' + ncart + ' ?',
      function(btn) {
        if (btn == 'yes') {
          this.getEl().mask('Annulation en cours, Veuillez patienter SVP ...');
          Ext.Ajax.request({
            url: Toc.CONF.CONN_URL,
            params: {
              module: 'users',
