...
class osC_Customers_Admin {
	function getData($id, $key = null) {
		...
		$Qcustomer = $osC_Database->query ( 'select c.*, ab.entry_gender as customers_gender,ab.entry_street_address as street_address,ab.entry_postcode as postcode,ab.entry_city as city,ab.entry_country_id as country_id,ab.entry_zone_id as z_code,ab.entry_fax as fax_number from :table_customers c left join :table_address_book ab on (c.customers_id = ab.customers_id) where c.customers_id = :customers_id' );
	...
	function save($id = null, $data, $send_email = false) {
		global $osC_Database;
        ...
        if (is_numeric($id)) {
            $Qcustomer = $osC_Database->query('update :table_customers set customers_gender = :customers_gender, customers_firstname = :customers_firstname, customers_lastname = :customers_lastname, customers_email_address = :customers_email_address, customers_dob = :customers_dob, customers_newsletter = :customers_newsletter, customers_groups_id = :customers_groups_id,customers_telephone = :customers_telephone, customers_status = :customers_status, date_account_last_modified = :date_account_last_modified where customers_id = :customers_id');
            $Qcustomer->bindRaw(':date_account_last_modified', 'now()');
            $Qcustomer->bindInt(':customers_id', $id);
        } else {
            $Qcustomer = $osC_Database->query('insert into :table_customers (customers_gender, customers_firstname, customers_lastname, customers_email_address, customers_dob, customers_newsletter, customers_status, customers_groups_id, number_of_logons, date_account_created,customers_telephone) values (:customers_gender, :customers_firstname, :customers_lastname, :customers_email_address, :customers_dob, :customers_newsletter, :customers_status, :customers_groups_id, :number_of_logons, :date_account_created,:customers_telephone)');
        ...
        $Qcustomer->bindReplace(':customers_groups_id', ($data['customers_groups_id'] == '') ? 'NULL'
                                                              : $data['customers_groups_id']);
        ...
        if ($error === false) {
            $Qp2c = $osC_Database->query('insert into :table_customers_to_groups (customers_id, customers_groups_id) values (:customers_id, :customers_groups_id)');
            $Qp2c->bindTable(':table_customers_to_groups', TABLE_CUSTOMERS_TO_GROUPS);
            $Qp2c->bindInt(':customers_id', $customer_id);
            $Qp2c->bindValue(':customers_groups_id', $data['customers_groups_id']);
            $Qp2c->execute();
        }
...
//			return $customer_id;
	...
	function addCustomerToGroup($customers_id, $groups_id) {
		global $osC_Database;
		
		$Qc2g = $osC_Database->query ( 'insert into :table_customers_to_groups (customers_id, customers_groups_id) values (:customers_id, :groups_id)' );
		$Qc2g->bindTable ( ':table_customers_to_groups', TABLE_CUSTOMERS_TO_GROUPS );
		$Qc2g->bindInt ( ':customers_id', $customers_id );
		$Qc2g->bindInt ( ':groups_id', $groups_id);
		$Qc2g->setLogging ( $_SESSION ['module'], $customers_id );
		$Qc2g->execute ();			
