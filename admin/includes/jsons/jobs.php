...
require('includes/classes/jobs.php');
...
class toC_Json_Jobs
{
    ...
    function listActionsTree()
    {
        global $toC_Json;

        $tree_node_cls = 'x-tree-node-collapsed';

        $records = array();

        $index = 0;

        $data = array('id' => $index++,'text' => 'Database','leaf' => true,'roles_id' => '','cls' => $tree_node_cls,'icon' => 'templates/default/images/icons/16x16/database.png');
        $records[] = $data;

        $data = array('id' => $index++,'text' => 'System','leaf' => true,'roles_id' => '','cls' => $tree_node_cls,'icon' => 'templates/default/images/icons/16x16/server_info.png');
        $records[] = $data;

        $data = array('id' => $index++,'text' => 'Security','leaf' => true,'roles_id' => '','cls' => $tree_node_cls,'icon' => 'templates/default/images/icons/16x16/locked.png');
        $records[] = $data;

        $data = array('id' => $index++,'text' => 'Network','leaf' => true,'roles_id' => '','cls' => $tree_node_cls,'icon' => 'templates/default/images/icons/16x16/world.png');
        $records[] = $data;

        $data = array('id' => $index++,'text' => 'Communication','leaf' => true,'roles_id' => '','cls' => $tree_node_cls,'icon' => 'templates/default/images/icons/16x16/email_unread.png');
        $records[] = $data;

        $data = array('id' => $index++,'text' => 'Automation','leaf' => true,'roles_id' => '','cls' => $tree_node_cls,'icon' => 'templates/default/images/icons/16x16/produce-expected.png');
        $records[] = $data;
...
//            ,'details' => toC_Reports_Admin::detailJob($Qreports->ValueInt('job_id'))
    ...
    function log() {
        ...
        $Qinsert->bindValue(':task_id',$task_id);
    ...
    function getJobStatus()
    {
        ...
        $query = "SELECT * FROM :table_logs where task_id = :task_id ORDER BY LOG_ID DESC LIMIT 0, 1";
        ...
        while ($Qreports->next()) {
            $records[] = array('task_id' => $task_id,
                ...
                'comment' => $Qreports->Value('comment')
            );
        }
