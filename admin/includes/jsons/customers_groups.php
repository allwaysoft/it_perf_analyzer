...
*/
  ...
  class toC_Json_Customers_Groups {
    ...
    function ListLastGroup() {
      global $toC_Json, $osC_Database, $osC_Language;

      $Qgroups = $osC_Database->query('select cg.customers_groups_id, cg.is_default, cgd.customers_groups_name from :table_customers_groups cg, :table_customers_groups_description cgd where cg.customers_groups_id = cgd.customers_groups_id and cgd.language_id = :language_id order by cg.customers_groups_id DESC LIMIT 1');
      $Qgroups->bindTable(':table_customers_groups', TABLE_CUSTOMERS_GROUPS);
      $Qgroups->bindTable(':table_customers_groups_description', TABLE_CUSTOMERS_GROUPS_DESCRIPTION);
      $Qgroups->bindInt(':language_id', $osC_Language->getID());
      $Qgroups->execute();
    ...
    function saveCustomersGroups() {
      ...
      $answer = toC_Customers_Groups_Admin::save((isset($_REQUEST['groups_id']) && is_numeric($_REQUEST['groups_id'] ) ? $_REQUEST['groups_id'] : null), $data) ;
      if ($answer == "true") {
