    ...
    class toC_Json_Customers
    {
        ...
        function listCustomers()
        {
            ...
            if (!empty ($_REQUEST ['categoryId'])) {
                $Qcustomers = $osC_Database->query('SELECT c.*,cgd.customers_groups_name FROM (:table_customers_to_groups ctg INNER JOIN :table_customers_groups_description cgd ON (ctg.customers_groups_id = cgd.customers_groups_id)) INNER JOIN :table_customers1 c ON (ctg.customers_id = c.customers_id) where cgd.language_id = :language_id and ctg.customers_groups_id = :categoryId');
            }
            else
            {
                $Qcustomers = $osC_Database->query('SELECT c.*,cgd.customers_groups_name FROM (:table_customers_to_groups ctg INNER JOIN :table_customers_groups_description cgd ON (ctg.customers_groups_id = cgd.customers_groups_id)) INNER JOIN :table_customers1 c ON (ctg.customers_id = c.customers_id) where cgd.language_id = :language_id');
            }
            ...
            while ($Qcustomers->next()) {
                ...
                <td>' . ($Qcustomers->value('customers_gender') == 'm' ? $osC_Language->get('gender_male')
                        : $osC_Language->get('gender_female')) . '</td>
              </tr>' . '<tr>
                ...
                $records [] = array('customers_id' => $Qcustomers->valueInt('customers_id'), 'customers_email_address' => $Qcustomers->value('customers_email_address'), 'customers_telephone' => $Qcustomers->value('customers_telephone'), 'customers_name' => $Qcustomers->value('customers_firstname') . ' ' . $Qcustomers->value('customers_lastname'), 'customers_lastname' => $Qcustomers->value('customers_lastname'), 'customers_firstname' => $Qcustomers->value('customers_firstname'), 'customers_credits' => $osC_Currencies->format($Qcustomers->value('customers_credits')), 'date_account_created' => osC_DateTime::getShort($Qcustomers->value('date_account_created')), 'customers_status' => $Qcustomers->valueInt('customers_status'), 'customers_info' => $customers_info);
            }
        ...
        function saveCustomer()
        {
            ...
            $data = array('gender' => (isset($_REQUEST['customers_gender']) ? $_REQUEST['customers_gender'] : ''),
                          ...
                          'city' => $_REQUEST['city'],
                          'state' => (isset($_REQUEST['z_code']) ? $_REQUEST['z_code'] : 'LT'),
                          'zone_id' => '0', //set blow
            ...
            if (strlen(trim($data ['firstname'])) < ACCOUNT_FIRST_NAME) {
                $error = true;
                $feedback [0] = sprintf($osC_Language->get('ms_error_first_name'), ACCOUNT_FIRST_NAME);
            }

            if (strlen(trim($data ['lastname'])) < ACCOUNT_LAST_NAME) {
                $error = true;
                $feedback [0] = sprintf($osC_Language->get('ms_error_last_name'), ACCOUNT_LAST_NAME);
            }

            if (strlen(trim($data ['email_address'])) < ACCOUNT_EMAIL_ADDRESS) {
                $error = true;
                $feedback [0] = sprintf($osC_Language->get('ms_error_email_address'), ACCOUNT_EMAIL_ADDRESS);
            ...
            if ((!isset ($_REQUEST ['customers_id']) || !empty ($data ['password'])) && (strlen(trim($data ['password'])) < ACCOUNT_PASSWORD)) {
                $error = true;
                $feedback [0] = sprintf($osC_Language->get('ms_error_password'), ACCOUNT_PASSWORD);
                ...
                $error = true;
                $feedback [0] = $osC_Language->get('ms_error_password_confirmation_invalid');
            }
        ...
        function getCustomersGroups()
        {
            ...
            $Qgroups = $osC_Database->query('select cg.customers_groups_id, cg.is_default, cgd.customers_groups_name from :table_customers_groups cg, :table_customers_groups_description cgd where cg.customers_groups_id = cgd.customers_groups_id and cgd.language_id = :language_id order by cgd.customers_groups_name ASC');
