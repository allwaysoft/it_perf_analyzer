<?php
    ...
    class osC_DashboardsTree
    {
        ...
        function osC_DashboardsTree($load_from_database = true)
        {
            ...
            if ($load_from_database === true) {
                ...
                if(isset($id))
                {
                    $query = "select c.categories_id,c.categories_status, c.parent_id, c.categories_image,c.content_type, cd.categories_name, cd.categories_url, cd.categories_page_title, cd.categories_meta_keywords, cd.categories_meta_description from delta_dashboard_folder c, delta_dashboard_folder_description cd where c.categories_id = cd.categories_id order by cd.categories_name";
                    ...
                    while ($Qcategories->next()) {
                        $this->data[$Qcategories->valueInt('parent_id')][$Qcategories->valueInt('categories_id')] = array('categories_id' => $Qcategories->valueInt('categories_id'), 'name' => $Qcategories->value('categories_name'),'url' => $Qcategories->value('categories_url'), 'page_title' => $Qcategories->value('categories_page_title'), 'meta_keywords' => $Qcategories->value('categories_meta_keywords'), 'meta_description' => $Qcategories->value('categories_meta_description'), 'image' => $Qcategories->value('categories_image'),'content_type' => $Qcategories->value('content_type'), 'count' => 0);
                    }
        ...
        function buildExtJsonTreeArrayForUser($parent_id = 0, $tree_node_cls = 'x-tree-node-collapsed', $include_templates_pages = false, $show_home = true, $check_permissions = false)
        {
            global $osC_Database;
            ...
            if (isset($this->data[$parent_id])) {
                if ($parent_id == 0 && $show_home == true) {
                    $permission = osC_Access::getUserPermissions($roles, -1,'pages');
                    $data = array('id' => -1, 'text' => 'Accueil');
                    $data['leaf'] = true;
                    $data['cls'] = $tree_node_cls;
                    $data['icon'] = 'templates/default/images/icons/16x16/home.png';
                ...
                foreach ($this->data[$parent_id] as $category_id => $category) {
                    if(!isset($category['administrators_id']))
                    {
                        $permission = osC_Access::getUserPermissions($roles, $category_id,$category['content_type']);
                    }
                    else
                    {
                        if($category['administrators_id'] == $_SESSION['admin']['id'])
                        {
                            $permission = array();
                    ...
                    $data = array('id' => $category_id,'content_type' => $category['content_type'], 'text' => $category['name'] . ($this->show_category_product_count === true
                            ? $this->category_product_count_start_string . $category['count'] . $this->category_product_count_end_string
                            : ''));

                    $data['icon'] = 'templates/default/images/icons/16x16/' . strtolower($category['content_type']) . '.png';

                    if (isset($this->data[$category_id])) {
                        if(isset($category['administrators_id']) && ($category['administrators_id'] == $_SESSION['admin']['id']))
                        {
     ...
     */
    class toC_DashboardsTree extends osC_DashboardsTree
    {
        var $leading_string = '';
